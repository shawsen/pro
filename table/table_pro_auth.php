 ...
 * 用户权限表
 **/
class table_pro_auth extends discuz_table
{
    public function __construct() {
		$this->_table = 'pro_auth';
		$this->_pk = 'uid';
		parent::__construct();
	}
    ...
    public function set()
    {
        ...
        $auth = pro_validate::getNCParameter('auth','auth','integer');
        $ctime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO ".DB::table('pro_auth')." (uid,auth,ctime) VALUES ('$uid','$auth','$ctime') ".
	...
	public function query()
	{
        ...
        $auth  = pro_validate::getNCParameter('auth','auth','integer');
        $key   = pro_validate::getNCParameter('key','key','string',128);
        $sort  = pro_validate::getOPParameter('sort','sort','string',1024,'uid');
...
EOF;
        ...
        foreach ($return["root"] as &$row) {
            $row['groupname'] = C::m('#pro#pro_usergroup')->grouptitle($row['groupid']);
