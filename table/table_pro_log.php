 ...
 * 用户日志表
 **/
class table_pro_log extends discuz_table
{
	...
	{/*{{{*/
		...
		$sday  = pro_validate::getNCParameter('sday','sday','string'); 
		$eday  = pro_validate::getNCParameter('eday','eday','string'); 
		$key   = pro_validate::getNCParameter('key','key','string'); 
        $sort  = pro_validate::getOPParameter('sort','sort','string',1024,'logtime');
		...
		$table_pro_log = DB::table($this->_table);
		$table_common_member = DB::table('common_member');
		$sql = <<<EOF
SELECT SQL_CALC_FOUND_ROWS a.*,b.username,b.email
FROM $table_pro_log as a LEFT JOIN $table_common_member as b ON a.uid=b.uid
...
FROM $table_pro_log WHERE $where GROUP BY vday ORDER BY logtime ASC
EOF;
