define(function(require){
    ...
    function getRow(im) {
        var pourl = '#/po/form~poid='+im.poid_code;
        var row = '<tr>'+
            '<td><a href="'+pourl+'" class="grida">['+im.poid+'] '+im.poname+'</a></td>'+
            '<td width="70" align="center">'+prStateRender(im.status)+'</td>'+
            '<td width="90" style="color:gray;">'+im.ctime.substr(0,10)+'</td>'+
            '<td style="text-align:right;width:50px;">'+
              '<a class="mwt-btn mwt-btn-primary mwt-btn-xs" href="'+pourl+'">查看</a></td>'+
        '</tr>';
        return row;
    };
	...
	o.init=function(domid){
        ...
        for (var i=0; i<tabs.length; ++i) {
            var tab = tabs[i];
            var url = "#/requirectl/mypos~s="+tab[0];
            var cls = i==0 ? ' class="mwt-active"' : '';
            var code = '<li'+cls+' name="po-tab" data-id="'+i+'"><a href="javascript:;">'+tab[0]+'</a></li>';
            tabcode.push(code);
        }
        var code = '<ul class="mwt-nav mwt-nav-tabs">'+
                '<li><span><i class="icon icon-order"></i> PO单</span></li>'+
                tabcode.join('')+
                '<li class="more"><a href="javascript:;" id="more-'+domid+'">more</a></li>'+
            '</ul>'+
            '<div id="'+gridid+'" style="min-height:200px;"></div>';
		jQuery('#'+domid).html(code);
        jQuery('[name=po-tab]').unbind('click').click(function(){
            jQuery('[name=po-tab]').removeClass('mwt-active');
